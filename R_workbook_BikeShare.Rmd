...
```{r}
DF_Final$started_at <- as.numeric(DF_Final$started_at)
```
...
```{r}
DF_Final$year <- strftime(DF_Final$started_at, "%Y")
DF_Final$month <- strftime(DF_Final$started_at, "%m")
```

```{r}
DF_Final$new_date <- as.POSIXct(paste(DF_Final$started_at), format = "%m:%d:%Y %H:%M:%S") 
