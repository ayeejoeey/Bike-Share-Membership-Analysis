...
```{r}
DF_Final$Date <- as.numeric(DF_Final$started_at)
```
...
```{r}
DF_Final$Date <- as.POSIXct(as.character(DF_Final$Date), origin='1970-01-01')
```

```{r}
DF_Final$year <- strftime(DF_Final$Date, "%Y")
DF_Final$month <- strftime(DF_Final$Date, "%m")
```

```{r}
DF_Final$new_date <- strftime(paste(DF_Final$Date), format = "%m:%Y") 

```

Note: date and time have been separated from "started_at". Now just need to formate "Date" to just month and year to continue analysis
...
##### To begin my analysis, I want to start looking at simple comparisons between casual and member memberships.

```{r}
casual_memb <- DF_Final %>%
...
casual_memb%>%
  group_by(date) %>%
  summarise(avg = round(mean(ride_length)/60))
```
